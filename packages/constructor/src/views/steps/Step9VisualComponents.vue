...
const FALLBACK_TYPES: ComponentTypeWithEmoji[] = [
  { id: 'ct_header', name: 'Header', description: 'Top header with logo and navigation', sort_order: 1, created_at: '', emoji: '🎯' },
  { id: 'ct_banners', name: 'Banners', description: 'Promotional banners displayed on the main page', sort_order: 2, created_at: '', emoji: '🎨' },
  { id: 'ct_thumbnails', name: 'Thumbnails', description: 'Game and category thumbnail cards', sort_order: 3, created_at: '', emoji: '🖼️' },
  { id: 'ct_tabbar', name: 'Tabbar', description: 'Bottom navigation tab bar', sort_order: 4, created_at: '', emoji: '📱' },
  { id: 'ct_sidebar', name: 'Sidebar', description: 'Side navigation panel', sort_order: 5, created_at: '', emoji: '📋' },
  { id: 'ct_theme', name: 'Theme', description: 'Light or dark color theme', sort_order: 6, created_at: '', emoji: '🎭' },
];

const INCOMPATIBLE_PAIRS: Array<{ typeIdA: string; variantNumA: number; typeIdB: string; variantNumB: number }> = [
...
<template>
  <div class="flex flex-col gap-6">
    ...
    <div class="border border-black/10 rounded-[10px] overflow-hidden">
      ...
      >
        <svg class="size-6 text-primary shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="w-14 h-7 rounded-full transition-colors relative shrink-0"
    ...
    <div v-else class="flex flex-col gap-2" :class="{ 'opacity-40 pointer-events-none': isDelegated }">
      <div
      ...
      >
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span class="text-base font-medium text-foreground tracking-[-0.31px]">{{ type.name }}</span>
            <span
            ...
            <svg
              v-if="selections[type.id] && hasConflicts && conflicts.some(c => c.message.includes(type.name))"
          ...
          >
            <div class="p-3 space-y-2">
              ...
              <button
                ...
                :class="selections[type.id] === variant.id
                ...
                <div class="flex items-center gap-3">
                  <div class="w-16 h-16 rounded-lg overflow-hidden bg-[#ececf0] flex-shrink-0">
                    ...
                    <div v-else class="w-full h-full flex items-center justify-center text-muted-foreground">
                      <svg class="size-6 opacity-30" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  ...
                  <div class="flex-1">
                    <div class="flex items-center justify-between">
                      <span class="text-sm font-medium text-foreground tracking-[-0.15px]">{{ variant.name }}</span>
                      <svg
              ...
              <!-- No variants -->
              <div
                v-if="!loadingVariants[type.id] && (!variantsByType[type.id] || variantsByType[type.id].length === 0)"
    ...
    >
      <div class="bg-[#fff8dc] px-4 py-3">
        <div class="flex items-center gap-3 mb-1">
          <svg class="size-6 text-[#d97706] shrink-0" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            <path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3" />
            <path d="M12 9v4" /><path d="M12 17h.01" />
          </svg>
          <span class="text-base font-medium text-foreground tracking-[-0.31px]">Несумісні компоненти</span>
        </div>
        <p
