...
app.post('/types', requireLibraryAccess('component_types'), async (c) => {
  ...
  ).bind(id, parsed.data.name, parsed.data.description, parsed.data.sort_order).run();
...
app.get('/types/:typeId/variants', async (c) => {
  ...
  const type = await c.env.DB.prepare('SELECT * FROM component_types WHERE id = ?').bind(typeId).first();
  if (!type) {
...
app.post('/types/:typeId/variants', requireLibraryAccess('component_variants'), async (c) => {
  ...
  if (!variantNumber) {
    ...
    ).bind(typeId).first<{ max_num: number | null }>();
  ...
  ).bind(id, typeId, parsed.data.name, variantNumber, user.id).run();

  const variant = await c.env.DB.prepare('SELECT * FROM component_variants WHERE id = ?').bind(id).first();
...
app.put('/variants/:id', requireLibraryAccess('component_variants'), async (c) => {
  ...
  const existing = await c.env.DB.prepare('SELECT * FROM component_variants WHERE id = ?').bind(id).first();
  if (!existing) {
  ...
  await c.env.DB.prepare(
    `UPDATE component_variants SET ${updates.join(', ')} WHERE id = ?`
  ).bind(...values).run();

  const updated = await c.env.DB.prepare('SELECT * FROM component_variants WHERE id = ?').bind(id).first();
