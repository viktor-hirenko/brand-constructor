...
### Чому Google OAuth + власний JWT?

Google OAuth забезпечує надійну автентифікацію через корпоративні Google-акаунти. Власний JWT дає повний контроль над часом дії сесії, даними користувача та перевірками дозволів.
...
| Мінімальні розміри | Залежать від типу асета (від 64×64 до 300×300) |
...
| Thumbnail компонента (`component_thumbnail`) | 200×40 | Без обмежень |

> **Примітка**: Строгі співвідношення сторін (16:9 для банерів, 1:1 для лого тощо) описані в продуктовій документації для **Phase 2 — Live Preview** у конструкторі брендів. В адмін-панелі (Phase 1) ratio не перевіряється, оскільки зображення відображаються через `object-fit: cover` у CSS.
