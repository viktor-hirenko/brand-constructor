...
<template>
  <div
    ...
    class="internal-naming-grid grid grid-cols-3 gap-2 max-w-[506px]"
  >
    <div
      v-for="naming in namings"
      :key="naming.id"
      class="internal-naming-grid__card relative h-[128px] rounded-[16px] bg-white flex flex-col items-center justify-center px-6 py-10 transition-all"
      :class="[
        selectedId === naming.id
          ? 'internal-naming-grid__card--selected border-2 border-[#030213] cursor-pointer'
          : disabled
            ? 'internal-naming-grid__card--disabled border border-black/10 opacity-40 cursor-not-allowed'
            : 'border border-black/10 hover:border-primary/50 cursor-pointer',
      ]"
      @click="handleClick(naming)"
    >
      <p
        class="internal-naming-grid__card-name text-[20px] font-medium leading-6 text-center text-[#1a1a1a] tracking-[-0.1504px] break-words w-full"
      >
        {{ naming.name }}
      </p>
  ...
  <div v-else class="internal-naming-grid__empty text-center py-8 text-muted-foreground text-sm">
