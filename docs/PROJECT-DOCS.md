...
| Tolerance (допуск aspect ratio) | 15% |
...
|---|---|---|
| Візуал концепту (`concept_visual`) | 300×300 | Без обмежень |
| Лого концепту (`concept_logo`) | 64×64 | Без обмежень |
| Thumbnail компонента (`component_thumbnail`) | 200×40 | Динамічно за типом компонента (див. нижче) |

### Aspect ratio за типом компонента

Значення зняті з прототипу (Figma) відповідно до вимог задачі. Перевірка опціональна: якщо значення = 0 — валідація не виконується. SVG-файли не перевіряються на розміри та ratio.

| Тип компонента | ID | Еталонні розміри | Aspect ratio | Мін. розміри |
|---|---|---|---|---|
| Header | `ct_header` | 387×64 | 6.05:1 | 100×20 |
| Banners | `ct_banners` | 273×144 | 1.9:1 | 100×50 |
| Thumbnails | `ct_thumbnails` | 270×126 | 2.14:1 | 100×50 |
| Tabbar | `ct_tabbar` | 289×57 | 5.07:1 | 100×20 |
| Sidebar | `ct_sidebar` | 289×614 | 0.47:1 | 100×100 |
| Theme | `ct_theme` | — | Без перевірки | — |

> **Примітка**: Конкретні ліміти розміру файлів (10 МБ PNG, 2 МБ SVG), мінімальні розміри та tolerance (15%) — рішення, прийняті при реалізації. Продуктова документація визначає лише необхідність перевірки aspect ratio за типом асета з тестовими значеннями з прототипу, підтримку PNG/SVG та відхилення невідповідних файлів. Значення можна змінити у `packages/shared/src/constants/assets.ts`.
