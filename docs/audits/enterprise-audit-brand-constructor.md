...
- **Next:** F-05 cleanup follow-up **DONE 2026-05-21 (wave 3)** — Bearer backward-compat removed from worker, `token` field dropped from `/api/auth/google`, legacy `localStorage` cleanup removed from both auth stores, `authMethod: 'bearer'` enum removed. Worker version `a4aeda65-4585-4a84-b246-152828a4dd9a`; admin bundle `index-Ddj9MF_N.js`; constructor bundle redeployed symmetrically. Smoke item #6 (CSRF on mutating POST) confirmed green by user (POST naming → 201). Optional remaining smoke: #7 private window, #9 manual fetch, #10 XSS surface, #11–17 PO+CEO E2E, Safari.
...
- _(F-05 cleanup, DONE 2026-05-21 wave 3)_: Bearer backward-compat fallback removed from `packages/worker/src/middleware/auth.ts`, `token` field removed from `/api/auth/google` response in `packages/worker/src/routes/auth.ts`, `localStorage.removeItem(LEGACY_STORAGE_KEY)` blocks removed from both `stores/auth.ts` files, `authMethod: 'cookie' | 'bearer' | 'dev'` narrowed to `'cookie' | 'dev'` in `types.ts` and the matching skip removed from `middleware/csrf.ts`. Deployed: worker `a4aeda65`, admin `index-Ddj9MF_N.js`, constructor redeployed symmetrically.
...
### Production deploy result — 2026-05-21 (wave 3: F-05 cleanup)

- **Worker deploy:** ✅ `wrangler deploy --env production`. Version ID `a4aeda65-4585-4a84-b246-152828a4dd9a`. Removed Bearer backward-compat fallback, dropped `token` from `/api/auth/google` response, narrowed `authMethod` to `'cookie' | 'dev'`, removed CSRF skip for legacy Bearer clients.
- **Admin Pages deploy:** ✅ bundle `index-Ddj9MF_N.js`. Removed legacy `localStorage.removeItem('brand_constructor_auth')` cleanup block from auth store.
- **Constructor Pages deploy:** ✅ bundle `index-Df816GGn.js`. Same localStorage cleanup removal applied symmetrically.
- **Smoke:** item #6 (CSRF on mutating POST — create naming → 201) confirmed green by user screenshots. Items #5 and #8 from wave 2 remain green.
