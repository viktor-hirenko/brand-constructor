...
- **Completed findings:** F-02 ✅, F-01 ✅, F-03 ✅, F-14 ✅, F-17 ✅, F-20 ✅, F-12 ✅, F-13 ✅
- **In progress:** —
- **Phase 1 COMPLETE — deploy when ready**
- **Next recommended finding (Phase 2):** F-15 — сузить deep:true watch в BrandPreviewPanel.vue
...
- **Model used:** Sonnet Thinking (Cursor Agent)
- **Files changed:**
  - `packages/constructor/src/stores/constructor.ts` — import `{ apiPatch, apiPost, apiPut }` (removed `getAuthHeader`); `saveBrand()` rewrites two raw `fetch` blocks to `apiPut`/`apiPost`; добавлен локальный `SaveBrandResult` interface
  - `packages/constructor/src/composables/useBrandPreviewLayers.ts` — import `{ getAssetUrl, apiGet }` (removed `getAuthHeader`); `loadVariants()` loop rewrites `fetch` → `apiGet`
  - `packages/constructor/src/views/steps/Step10ReviewSubmit.vue` — добавлен `apiGet` в существующий import строки 6; убран дублирующий `import { getAuthHeader }` строки 20; `loadComponentSelectionDetails()` и `handlePrintBrand()` rewrites 2×fetch → `apiGet`
- **Summary of changes:** Все ручные `fetch()`-вызовы в constructor-пакете переведены на `apiPost`/`apiPut`/`apiGet` из `useApi.ts`. Auth header, Content-Type, API_BASE и error-throw теперь обрабатываются централизованно в `request()`. Удалено ~30 строк boilerplate.
- **Verification commands run:**
  ```
  pnpm --filter @brand-constructor/constructor type-check   # ✓ 0 errors (4.1s)
  ```
- **Verification result:** 0 ошибок TypeScript.
- **Manual QA after deploy:** Сохранение черновика (Step 1→Step 9), submit (Step 10), PDF-print — все сетевые вызовы должны работать идентично.
- **Suggested commit message:**
  ```
  refactor(constructor): replace raw fetch() with apiPost/apiPut/apiGet

  saveBrand() in constructor.ts, loadVariants() in useBrandPreviewLayers.ts,
  loadComponentSelectionDetails() and handlePrintBrand() in Step10ReviewSubmit.vue
  all used manual fetch() with duplicated auth headers, API_BASE construction and
  error handling. Replaced with apiPost/apiPut/apiGet from useApi composable.
...
- **Model used:** Sonnet Thinking (Cursor Agent)
- **Files changed:**
  - `packages/constructor/src/stores/libraries.ts` — новый Pinia store `useLibrariesStore` с TTL-кэшем 30s; одна точка загрузки concepts / externalNamings / internalNamings / prPackages; `load(brandId, force?)` — идемпотентен при повторном вызове в рамках TTL
  - `packages/constructor/src/views/ConstructorLayout.vue` — убраны 3 `useApiList` + `fetchConcepts/fetchExternalNamings/fetchInternalNamings`; `concepts/externalNamings/internalNamings` теперь computed из стора; `loadPreviewData()` вызывает `librariesStore.load(brandId)` с TTL-защитой
  - `packages/constructor/src/views/steps/Step10ReviewSubmit.vue` — убраны 4 `useApiList` декларации + 4 `fetchXxx()` вызова из `onMounted`; 4 computed (`concepts/externalNamings/internalNamings/prPackages`) читают из стора; `onMounted` вызывает `librariesStore.load(store.brandId)` — если ConstructorLayout уже загрузил в течение 30s, вызов — no-op
- **Summary of changes:** До фикса Step10 при каждом монтировании делал 4 отдельных запроса к тем же endpoints, что уже загрузил ConstructorLayout при переходе на Step 8. Теперь все 4 списка управляются централизованно через `useLibrariesStore` с TTL 30s. При повторном монтировании Step10 (возврат из CEO-reselect / PO-edit) лишних запросов нет.
- **Verification commands run:**
  ```
  pnpm --filter @brand-constructor/constructor type-check   # ✓ 0 errors (4.2s)
  ```
- **Verification result:** 0 ошибок TypeScript.
- **Manual QA — NEEDS_VERIFICATION:** Открыть Network вкладку DevTools, пройти шаги 1→8→10: убедиться что `/api/concepts`, `/api/namings/external`, `/api/namings/internal`, `/api/pr-packages` не вызываются повторно при маунте Step10. Если items Step10 пустые — timing issue: ConstructorLayout не успел загрузить к маунту Step10; в этом случае `librariesStore.load()` сам загрузит.
- **Suggested commit message:**
  ```
  perf(constructor): deduplicate library fetches via useLibrariesStore TTL cache

  Step10ReviewSubmit re-fetched concepts/externalNamings/internalNamings/prPackages
  on every mount even though ConstructorLayout already loaded them at step 8.
  New useLibrariesStore (stores/libraries.ts) centralises all four lists with a
  30-second TTL. Both ConstructorLayout and Step10 read from the store; the second
  load() call within the TTL window is a no-op.
