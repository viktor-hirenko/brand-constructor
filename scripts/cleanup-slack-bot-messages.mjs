...
function loadToken() {
  ...
  console.error(
    '❌  SLACK_BOT_TOKEN не знайдено. Передайте через env або переконайтесь, що packages/worker/.dev.vars існує.'
  )
  process.exit(1)
}
...
async function main() {
  ...
  for (const channel of CHANNELS) {
    ...
    try {
      ...
      console.error(
        '     Можлива причина: бот не є членом каналу або відсутній scope channels:history / groups:history.'
      )
      continue
    }
    ...
    console.log(
      `  📨  Всього повідомлень: ${messages.length}, повідомлень бота: ${botMessages.length}`
    )
