...
- **In progress:** F-06 (Step10 decomposition, 3 PRs)
- **Phase 1 COMPLETE ✅ — deployed to prod 2026-05-20**
- **Phase 2 COMPLETE ✅ — deployed to prod 2026-05-20**
- **New findings:** F-21 (added 2026-05-20, found during F-19 verification), F-22 (added 2026-05-20, found during F-06 PR #1 — full removal of unused `rejected` status from stack) — see below
- **Next:** Phase 3 — только Opus 4.7, только по явному запросу. F-06 in progress.
...
- **Notes:** Touches all five active review modes (CEO finalize, PO draft, PO submitted, PO returned-from-CEO, approved read-only). The legacy "default" `v-else` view that only rendered on `status === 'rejected'` is removed in PR #1 — that flow was inherited from the original v1 PRD (when CEO had an explicit "Reject" action) and is no longer reachable from the current UI / Figma. The `rejected` status itself is **not** handled by the constructor anymore (no defensive fallback) since the product confirmed it never occurs in the current flow; systematic removal of `rejected` from the worker / shared types / admin filter is tracked as F-22.
- **PR plan:**
  - **PR #1** — **remove** the legacy `ReviewLegacyView` branch entirely, drop all CEO/PO computeds and scroll save/restore mechanisms used only by the legacy view, extract the `componentSelectionDetails` loader into `composables/useReviewComponentSelections.ts` (still needed by the unified view's Visual Components summary and by the PDF generator).
  - **PR #2** — extract unified review template into `components/constructor/review/ReviewUnifiedView.vue` along with its layout-specific computeds (`reviewHeaderInfoOverride`, `unifiedReviewTitle/Subtitle`, `isPoEditable`, `externalNamingItems`, `deliverablesScope/TimingText`, `visualComponentsSummary`, `brandHeaderTitle`, `getPoCommentForSection`, etc.).
  - **PR #3** — extract orchestration composables `useCeoReviewComments.ts` (ceoComments reactive mirror, revision validation, comment/resolve handlers) and `useCeoApplyVariants.ts` (CEO selection helpers, applied-state computeds, attention counter, dependency-guard modals, apply-variant handlers). Final `Step10ReviewSubmit.vue` becomes a thin orchestrator (≤500 lines).
...
- **Notes:** Coordinated worker + frontend + constructor + shared change; **needs explicit user confirmation that there are no `rejected` rows in production D1** before tightening the worker Zod enum (otherwise old rows become unreadable). Until F-22 lands, the constructor renders `rejected` as the default `'po-draft'` mode (which is wrong but unreachable in practice).
...
- **Notes:** Was intentionally **out of scope** for F-19 (which was strictly the 404→403 owner check). Logged here so a future Sonnet pass can close it without re-discovering. Do **not** combine with F-06 in the same chat.
