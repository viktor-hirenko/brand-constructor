...
app.post('/', requireLibraryAccess('pr_packages'), async (c) => {
  ...
  const exists = await c.env.DB.prepare(
    'SELECT id FROM pr_packages WHERE number = ?'
  ).bind(parsed.data.number).first();

  if (exists) {
    return c.json({ success: false, error: `PR package #${parsed.data.number} already exists` }, 409);
  }
...
app.put('/:id', requireLibraryAccess('pr_packages'), async (c) => {
  ...
  await c.env.DB.prepare(
    `UPDATE pr_packages SET ${updates.join(', ')} WHERE id = ?`
  ).bind(...values).run();
