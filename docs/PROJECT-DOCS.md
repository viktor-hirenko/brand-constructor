...
brand-constructor/
├── packages/
│   ├── admin/             # Vue 3 SPA — інтерфейс адмін-панелі
...
|---|---|
| `admin` | Vue 3 SPA. Весь UI адмін-панелі: вьюшки, компоненти, стори, роутинг, стилі |
| `constructor` | Vue 3 SPA. Brand Constructor — wizard створення бренду для Product Owners |
| `worker` | Cloudflare Worker. REST API з авторизацією, CRUD-роути, завантаження асетів у R2, запити до D1 |
| `shared` | TypeScript-інтерфейси, константи та дозволи, що використовуються усіма пакетами |
...
Локальна розробка використовує **той самий** Google OAuth flow, що й продакшн (з 2026-05-21, F-25). Раніше існував bypass через `ENVIRONMENT=development` + заголовок `X-Dev-User-Email` — він повністю видалений, оскільки був головним джерелом ризику з зовнішнього security review (Anton Zasteba).

Як запускати локально:

1. Покласти у `packages/worker/.dev.vars` секрети (`JWT_SECRET`, `GOOGLE_CLIENT_ID`, опційно `PANANAMES_SIGNATURE`, `SLACK_BOT_TOKEN`, ...).
2. `wrangler dev --env production` у `packages/worker/`.
3. `pnpm --filter @brand-constructor/constructor dev` (або `admin`) у іншому терміналі.
4. У браузері відкрити `http://localhost:5174` → "Sign in with Google" → залогінитися реальним Google-акаунтом, чий email присутній у `users` таблиці.

> `http://localhost:5173` і `:5174` мають бути додані у Authorized JavaScript origins у Google Cloud Console для production `GOOGLE_CLIENT_ID`. Це одноразове налаштування.
...
| POST | `/auth/logout` | Так | Вихід із системи |
...
| Метод | Ендпоінт | Опис |
|---|---|---|
| GET | `/components/types` | Список типів компонентів з кількістю варіантів |
...
| POST | `/assets/upload` | Завантажити файл (потрібна авт.). FormData: `file`, `entity_type`, `entity_id`, `file_type`, `slot`, `component_type_id` (опц.), `aspect_ratio` (опц.) |
| DELETE | `/assets/:id` | Видалити асет за ID із бази та R2 |
...
| Параметр | Значення |
|---|---|
| Максимальний розмір файлу (PNG) | 10 МБ |
| Максимальний розмір файлу (SVG) | 2 МБ |
| Дозволені формати | PNG, SVG (визначення через magic bytes) |
| Мінімальні розміри | Залежать від типу асета (див. таблиці нижче) |

### Типи асетів та обмеження

| Асет | Мін. розміри | Aspect ratio |
|---|---|---|
| Візуал концепту (`concept_visual`) | 300×300 | Опціонально (поле вводу) |
| Лого концепту (`concept_logo`) | 64×64 | Опціонально (поле вводу) |
| Thumbnail компонента (`component_thumbnail`) | 200×40 | Опціонально (поле вводу з prefill) |

### Поле "Співвідношення сторін"

При завантаженні зображень користувач може вказати очікуване співвідношення сторін у полі "Співвідношення сторін":

- **Поле заповнене** — виконується перевірка з tolerance ±1% від вказаного значення
- **Поле порожнє** — перевірка не виконується, дозволяється завантаження з будь-яким співвідношенням сторін
- **Формат вводу** — десяткове число (наприклад `1.9`) або пропорція (наприклад `16:9`)

Для UI-компонентів поле автоматично заповнюється значенням з конфігурації (prefill), яке користувач може змінити або очистити.

### Еталонні значення aspect ratio для UI-компонентів

Значення зняті з прототипу (Figma) і використовуються як prefill у полі вводу. SVG-файли не перевіряються на розміри та ratio.

| Тип компонента | ID | Еталонні розміри | Prefill aspect ratio | Мін. розміри |
|---|---|---|---|---|
| Header | `ct_header` | 387×64 | 6.05 | 100×20 |
| Banners | `ct_banners` | 273×144 | 1.9 | 100×50 |
| Thumbnails | `ct_thumbnails` | 270×126 | 2.14 | 100×50 |
| Tabbar | `ct_tabbar` | 289×57 | 5.07 | 100×20 |
| Sidebar | `ct_sidebar` | 289×614 | 0.47 | 100×100 |
| Theme | `ct_theme` | — | — (без prefill) | — |

> **Примітка**: Конкретні ліміти розміру файлів (10 МБ PNG, 2 МБ SVG) та мінімальні розміри — рішення, прийняті при реалізації. Значення можна змінити у `packages/shared/src/constants/assets.ts`.

### Поведінка при помилках валідації

- **API**: При невідповідності вимогам повертає HTTP 400 з текстом помилки (наприклад: "Image dimensions too small", "Invalid aspect ratio")
- **Frontend**: Показує помилку через нативний браузерний `alert()`
- **Підтвердження видалення**: Використовується нативний `confirm()`
...
Після F-26 (2026-05-21 security review) production-біндинги (D1, R2, vars, секрети, rate-limit) існують **тільки** у блоці `[env.production]`. Default (top-level) env навмисно не має жодних production-біндингів — це гарантія від випадкового `wrangler deploy` без `--env production`, який раніше міг винести production-D1 на публічний `*.workers.dev`.

```toml
# Production environment — єдина точка зв'язку з production D1 / R2
[env.production]
vars = { ENVIRONMENT = "production", CORS_ORIGINS = "https://brand-constructor.pages.dev,https://brand-constructor-app.pages.dev", ... }
...
> **Деплой:** завжди `wrangler deploy --env production`. Без `--env production` команда впаде з помилкою (немає `main` бінду у default env) — це навмисно.
> **Локально:** `wrangler dev --env production` з заповненим `.dev.vars`.
...
`@brand-constructor/shared` — внутрішній пакет, що імпортується пакетами `admin`, `constructor` та `worker`.
...
- **Utils**: `parseAspectRatio(input: string): number | null` — парсить співвідношення сторін з формату `"16:9"` або `"1.5"`
...
pnpm build:admin     # Збілдити лише admin
