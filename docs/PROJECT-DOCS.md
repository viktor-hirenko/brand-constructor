...
### 4.1.1 Хто може увійти (allow-list) — поточний стан і план

**Зараз (тимчасово, для тестування):**

- Доступ **не** обмежений доменом email на рівні коду.
- Після успішного Google OAuth Worker перевіряє лише наявність `email` у
  таблиці `users` (D1) — ручний allow-list, який наповнює **лише** `admin`
  через BC Admin → Users (`POST /api/users`, `requireAdminOnly`).
- Будь-який Google-акаунт, чий email додано в `users`, може увійти в обидва SPA
  (admin + constructor), незалежно від домена (`@upstars.com`, `@gmail.com`, …).
- У `seed.sql` для локальної розробки є лише `admin@brand-constructor.dev` —
  production-список живе в D1 і задається вручну (див. audit checklist у
  [`CLOUDFLARE-HARDENING.md`](./security/CLOUDFLARE-HARDENING.md) §7).

**Заплановано (production hardening):**

- Обмеження входу **співробітниками Upstars**: email з доменом **`@upstars.com`**
  (перевірка на Worker після Google tokeninfo **або** політика Cloudflare Access
  «emails ending `@upstars.com`» — див.
  [`CLOUDFLARE-HARDENING.md`](./security/CLOUDFLARE-HARDENING.md) §3–4).
- Додатково — Cloudflare Access перед Pages/Worker hostnames, щоб публічні
  `*.pages.dev` / `*.workers.dev` не були точкою входу без корпоративного IdP.

> **Для security review:** поточна модель — «whitelist у D1 + Google OAuth»,
> без domain gate. Це свідомий тимчасовий режим до впровадження `@upstars.com`
> + Access. Маршрути в §9–10 з позначкою «authenticated» означають «будь-хто з
> записом у `users`», а не «будь-хто в інтернеті».
...
8 ролей у `packages/shared/src/constants/roles.ts`. Доступ перевіряється на
backend (middleware); frontend лише ховає UI — backend завжди повторно валідує.

| Роль | Код | Перегляд | Wizard | Редагування брендів | Бібліотеки (CRUD) | User management (`/users`) |
|---|---|---|---|---|---|---|
| Admin | `admin` | усе | усе + новий бриф | усе | усе | так |
| Head of DHC | `head_dhc` | усе | CEO/огляд | усе | усе | ні |
| Product Owner | `product_owner` | усе | свої бренди (draft/submit) | свої до approve | — | ні |
| CPO / CEO | `cpo_ceo` | усе | approve / send-back | — | — | ні |
| Strategy & Identity | `strategy_identity` | усе | — | — | concepts + namings | ні |
| UI Designer | `ui_designer` | усе | — | — | component types/variants | ні |
| PR & Marketing | `pr_marketing` | усе | — | — | pr_packages | ні |
| Product Designer | `product_designer` | усе | — | — | — | ні |

Константи в `packages/shared/src/constants/roles.ts`:

- `LIBRARY_WRITE_PERMISSIONS` — library → дозволені ролі.
- `ADMIN_ROLES = [admin, head_dhc]` — операційний доступ (бренди, approve).
- `BRAND_APPROVAL_ROLES = [admin, head_dhc, cpo_ceo]` — CEO approval flow.
- `BRAND_BRIEF_CREATOR_ROLES = [product_owner, admin]` — створення нового брифа.
- User management endpoints (`/api/users` POST/PUT/DELETE) — окремий
  `requireAdminOnly` (лише `role === 'admin'`).
...
> **Доступ «authenticated»** = користувач є в таблиці `users` після Google OAuth
> (див. §4.1.1). Це **тимчасово** без обмеження `@upstars.com`; у production
> планується domain gate + Cloudflare Access.

| Route | View | Доступ (зараз) | RBAC / примітки |
|---|---|---|---|
| `/login` | `LoginView` | публічний | — |
| `/concepts`, `/concepts/:id` | `ConceptsView`, `ConceptDetailView` | authenticated + write за роллю | `LIBRARY_WRITE_PERMISSIONS.concepts` |
| `/namings` | `NamingsView` | authenticated + write за роллю | external/internal namings |
| `/pr-packages` | `PrPackagesView` | authenticated + write за роллю | `pr_packages` |
| `/components`, `/components/:typeId` | `ComponentsView`, `ComponentVariantsView` | authenticated + write за роллю | component types/variants |
| `/brands` | `BrandsView` | authenticated | список брендів — фільтр на API за роллю |
| `/users` | `UsersView` | **лише `admin`** (PRD) | API: `requireAdminOnly`. У UI router/sidebar ще показує пункт для `head_dhc` — розбіжність, див. §8.2 |
...
> **Доступ:** як у §4.1.1 — зараз будь-який email з записом у `users`; у
> майбутньому — `@upstars.com` + Cloudflare Access.

| Route / зона | Доступ (зараз) | RBAC |
|---|---|---|
| `/` (`LandingView`) | **публічний** (без login) | кнопка «Почати» / новий бриф — лише `product_owner` + `admin` (`canStartNewBrandBrief`) |
| `/login` | публічний | Google OAuth → whitelist у `users` |
| `/constructor/brand/:id/...` (wizard) | authenticated | PO — свій flow; `head_dhc` / `cpo_ceo` / `admin` — CEO-гілки за статусом |
| `.../ceo-reselect/*` | authenticated | `isCeoOrAdmin` (`admin`, `head_dhc`, `cpo_ceo`) + статус `submitted` / `needs_revision` |
| `.../po-edit/*` | authenticated | лише `product_owner`, статус `needs_revision` |
| `/constructor/success` | authenticated | `briefCreatorOnly` — PO / admin |
