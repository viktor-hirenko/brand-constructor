...
function buildSectionContent(title: string, rows: SectionRow[]): Content[] {
  ...
  return [
    ...
    {
      canvas: [
        { type: 'line', x1: 0, y1: 0, x2: 515, y2: 0, lineWidth: 1.5, lineColor: '#e5e5e5' },
      ],
...
export function usePrintBrand() {
  ...
  function buildDocDefinition(data: PrintBrandData): TDocumentDefinitions {
    ...
    // 1. Brand Basics
    content.push(
      ...buildSectionContent('Brand Basics', [
    ...
    // 2. Mode
    content.push(
      ...buildSectionContent('Mode', [
        {
          label: 'Тема',
          value: sd.mode === 'dark' ? 'Dark Mode' : sd.mode === 'light' ? 'Light Mode' : '—',
        },
      ])
    )
    ...
    if (sd.concept.newConceptBrief) {
      ...
      content.push(
        ...buildSectionContent('Концепт (новий бриф)', [
          ...
          {
            label: "Зв'язок з продуктами?",
            value:
              b.keepProductConnection === null
                ? '—'
                : b.keepProductConnection
                  ? `Так — ${b.connectedProducts}`
                  : 'Ні',
          },
    ...
    if (sd.externalNaming.newNamingBrief) {
      ...
      content.push(
        ...buildSectionContent('External Naming (новий бриф)', [
          { label: 'Нове ГЕО?', value: b.isNewGeo === null ? '—' : b.isNewGeo ? 'Так' : 'Ні' },
      ...
      content.push(
        ...buildSectionContent('External Naming', [
